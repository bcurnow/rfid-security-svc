...
@click.pass_context
def create(ctx, first_name, last_name):
    """Manually adds a record to the table."""
    try:
        model.create(first_name, last_name)
        ctx.invoke(list)
    except exception.DuplicateGuestError:
        ctx.fail(click.style(f'Record with first_name "{first_name}" or last_name "{last_name}" already exists.', fg='red'))
...
@click.argument('last_name')
@click.pass_context
def update(ctx, id, first_name, last_name):
    """Manually updates a record in the table."""
    try:
        model.update(id, first_name, last_name)
