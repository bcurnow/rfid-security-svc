...
from rfidsecuritysvc.db import media_perm as table, media, permission
...
def update_command(ctx, id, media_id, perm_id):
  ...
  click.echo(str(table.update(id, media_id, perm_id)) + ' record(s) updated')
  ctx.invoke(list_command)
