...
docker:
	...
	@if [[ -e /dev/input/rfid && -c /dev/input/rfid ]]; then \
	  docker run -it -p 5000:5000 --mount src=$(ROOT_DIR)/rfid-db,target=/rfid-db,type=bind--mount src="$(ROOT_DIR)",target=/rfid-security-svc,type=bind --device=/dev/input/rfid:/dev/input/rfid:r $(IMAGE_NAME):latest /bin/bash; \
	else \
	  echo "Did not find /dev/input/rfid, skipping --device"; \
	  docker run -it -p 5000:5000 --mount src=$(ROOT_DIR)/rfid-db,target=/rfid-db,type=bind --mount src="$(ROOT_DIR)",target=/rfid-security-svc,type=bind $(IMAGE_NAME):latest /bin/bash; \
	fi

docker-prod:
	@mkdir -p $(ROOT_DIR)/rfid-db
	docker run -p 5000:5000 --mount src=$(ROOT_DIR)/rfid-db,target=/rfid-db,type=bind $(IMAGE_NAME):production $(ARGS)
