...
def test_create_app(app):
    ...
    assert os.path.exists(app.instance_path) == True
...
def _assert_connexion_options(options):
    assert options['instance_relative_config']  == True
...
def _assert_swappger_ui_config(config):
    ...
    assert config['persistAuthorization'] == True

def _assert_api(api):
    assert api.strict_validation == True
    assert api.validate_responses == True
