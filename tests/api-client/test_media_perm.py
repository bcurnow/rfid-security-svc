...
def test_get(rh, media_perms):
    rh.assert_response(rh.open('get', f'{api}/media/{media_perms[0].media_id}/perm/{media_perms[0].perm_id}'), 200, media_perms[0])
...
@patch('rfidsecuritysvc.api.media_perm.model')
...
def test_post(rh, creatable_media_perm):
    ...
    rh.assert_response(rh.open('get', f'{api}/media/{p.media_id}/perm/{p.perm_id}'), 200)
    rh.assert_response(rh.open('delete', f'{api}/media/{p.media_id}/perm/{p.perm_id}'), 200, headers={RECORD_COUNT_HEADER: '1'})
    rh.assert_response(rh.open('get', f'{api}/media/{p.media_id}/perm/{p.perm_id}'), 404)
...
def test_delete(rh, creatable_media_perm):
    ...
    rh.assert_response(rh.open('get', f'{api}/media/{p.media_id}/perm/{p.perm_id}'), 200)
    rh.assert_response(
        rh.open('delete', f'{api}/media/{p.media_id}/perm/{p.perm_id}'), 200, headers={RECORD_COUNT_HEADER: '1'})
    rh.assert_response(rh.open('get', f'{api}/media/{p.media_id}/perm/{p.perm_id}'), 404)
...
def test_delete_notfound(rh, creatable_media_perm, permissions):
    ...
    rh.assert_response(rh.open('delete', f'{api}/media/{p.media_id}/perm/{p.perm_id}'), 200, headers={RECORD_COUNT_HEADER: '0'})


def test_put_create(rh, creatable_media_perm):
    rh.assert_response(rh.open('put', f'{api}/media/{creatable_media_perm.media_id}/perm/{creatable_media_perm.perm_id}',
                               creatable_media_perm), 201, headers={RECORD_COUNT_HEADER: '1'})
    rh.assert_response(rh.open('delete', f'{api}/media/{creatable_media_perm.media_id}/perm/{creatable_media_perm.perm_id}',
                               creatable_media_perm), 200, headers={RECORD_COUNT_HEADER: '1'})


def test_put_duplicate(rh, media_perms):
    rh.assert_response(
        rh.open('put', f'{api}/media/{media_perms[0].media_id}/perm/{media_perms[0].perm_id}', media_perms[0]),
        200,
