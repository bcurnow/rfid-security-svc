...
def test_put_no_content(rh, sounds):
    rh.assert_response(
        rh.open('put', f'{api}/{sounds[0].id}', ({'name': 'rename.wav'}, None), 'multipart/form-data'),
        415,
    )
...
def test_put_notfound(rh, creatable_sound):
    rh.assert_response(rh.open('put', f'{api}/{creatable_sound.id}', creatable_sound.test_to_multipart(), 'multipart/form-data'), 201, headers={RECORD_COUNT_HEADER: '1'})
...
def test_put_wrong_content_type(rh, creatable_sound):
    rh.assert_response(rh.open('put', f'{api}/{creatable_sound.id}', creatable_sound.test_to_multipart('application/wrong'), 'multipart/form-data'), 415)
