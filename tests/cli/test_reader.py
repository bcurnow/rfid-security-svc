...
def test_set_device_name(config, list, runner):
    ...
    list.return_value = []
    runner.invoke(args=['reader', 'set-device-name', '/dev/test'], color=True)
...
def test_set_device_name_alreadyexists(config, list, runner, assert_output):
    ...
    assert_output(result,
                  f'There is already a key "{RFID_DEVICE_CONFIG_KEY}" with value "/dev/othervalue", do you want to replace with "/dev/test"?',
                  0)
    config.get.assert_called_once_with(RFID_DEVICE_CONFIG_KEY)
